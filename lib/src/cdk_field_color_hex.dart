...
class CDKFieldColorHex extends StatefulWidget {
  ...
  final bool enabled;
  final ValueChanged<int>? onValueChanged; // Tipus de dades canviat a int
  ...
  @override
  State<CDKFieldColorHex> createState() => _CDKFieldColorHexState();
}

class _CDKFieldColorHexState extends State<CDKFieldColorHex> {
  ...
  void initState() {
    super.initState();
    _previousValue = widget.value & 0xFFFFFF;
  ...
  void _setCurrentValue(String text) {
    ...
    int parsedValue = _parseHexValue(newValue, fallback: _previousValue);
    bool valueChanged = parsedValue != _previousValue;

    _controller.text = _fixText(parsedValue);
  ...
  void _onTextChanged() {
    ...
    int parsedValue = _parseHexValue(hexValue, fallback: _previousValue);
    widget.onTextChanged?.call(parsedValue);
  }

  int _parseHexValue(String hexValue, {required int fallback}) {
    if (hexValue.isEmpty) {
      return fallback;
    }
    return int.tryParse(hexValue, radix: 16) ?? fallback;
  }
